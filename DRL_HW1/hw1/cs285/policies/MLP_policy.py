...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def update(self, observations, actions):
        ...
        # action = distributions.Categorical(torch.tensor(actions)).sample() # 안됨 : 첫번째 시도 (categorical 로 뽑기) 실패
        action = distributions.Normal(torch.tensor(actions), torch.exp(self.logstd)).sample()
        # action = torch.tensor(actions).long()
        loss = F.cross_entropy(self.forward(observations), action)
