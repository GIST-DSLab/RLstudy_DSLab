        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    ...
                    paths[i]["action"] = get_action(paths[i]["observation"], policy = expert_policy)
...
def get_action(obs, policy):
    """
    Args:
        obs: (torch.FloatTensor) shape: (batch_size, obs_dim)

    Returns:
        (torch.FloatTensor): shape: (batch_size, ac_dim)
    """
