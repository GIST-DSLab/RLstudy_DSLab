...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        mean_actions = self.mean_net(torch.Tensor(observation))

        # Create a multivariate normal distribution with mean_actions and logstd as the diagonal covariance matrix
        action_distribution = torch.normal(mean_actions, torch.exp(self.logstd))
    ...
    def update(self, observations, actions):
        ...
        loss = F.cross_entropy(distributions, torch.Tensor(actions))
